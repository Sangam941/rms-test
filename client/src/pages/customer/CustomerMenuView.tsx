...
const CustomerMenuView: React.FC = () => {
  ...
  // Track current/last confirmed order
  ...
  useEffect(() => {
    fetchAll().then(() => setAllItems(getFilteredItems()));
  }, []);

  // Keep items in sync with store (on filter/category/search change)
  useEffect(() => {
    setAllItems(getFilteredItems());
  }, [getFilteredItems]);

  // Find active order for this table (if any)
  // orders here: CustomerOrder[], not "Order"
                      ...
                      >
                        ...
                        <div className="mb-4">
                          <div className="flex justify-between mb-2">
                            <div
                              className={`flex flex-col items-center ${order.status === "pending" ||
                            ...
                            <div
                              className={`flex flex-col items-center ${order.status === "preparing" ||
                                  order.status === "completed"
                                  ? "text-orange-600"
                                  : "text-gray-400"
                                }`}
                            ...
                            <div
                              className={`flex flex-col items-center ${order.status === "completed"
                                  ? "text-gray-600"
                                  : "text-gray-400"
                                }`}
                            >
                          ...
                          <div className="w-full bg-gray-200 h-2 rounded-full">
                            <div
                              className={`h-2 rounded-full transition-all duration-500 ${order.status === "completed"
                                  ? "bg-gray-600 w-full"
                                  : order.status === "preparing"
                                    ? "bg-orange-600 w-1/2"
                                    : "bg-blue-600 w-1/4"
